<?php
	...
	function invserseScaleImage($srcUrl, $boxW, $boxH) {
		list($width, $height, $type, $attr) = getimagesize($srcUrl);
		$orig = imagecreatefromfile($srcUrl);
		print("width=$width, height=$height\n");
	...
	function cropToSquare($thumbw, $thumbh,  $meta) {
		$tx = 0; $ty = 0; $tw = $thumbw;  $th = $thumbh;
		$dw = $meta['width'] - $thumbw;
		$dh = $meta['height'] - $thumbh;
		$tx = $dw / 2.0;
		$ty = $dh / 2.0;

		$thumb = imagecreatetruecolor($thumbw, $thumbh);
		imagecopy($thumb,  $meta['phpimg'], 0, 0, $tx, $ty, $thumbw, $thumbh);
	...
	function doIteration() {
		...
		foreach($assets as $asset) {
			...
			$meta2 = invserseScaleImage($imageUrl, 300, 300);
			print("meta2:");
			print_r($meta2);
			$meta = cropToSquare(300, 300, $meta2);
			print("meta3:");
			print_r($meta);
			imagejpeg($meta['phpimg'], $outputFilename,100);
